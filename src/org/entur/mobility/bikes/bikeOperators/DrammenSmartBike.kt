        ...
        data = StationsInformation(
            stations.map {
                StationInformation(
                    ...
                    name = it.name,
                    capacity = statusResponse?.data?.stations?.find { status -> status.station_id == mapIdToNeTEx(it.id, Operator.DRAMMENBYSYKKEL) }
                        .let { station -> (station?.num_bikes_available ?: 0) + (station?.num_docks_available ?: 0) }
                )
            }
        )
    )
